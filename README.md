💱 Currency Converter (Frontend with Live API)

📌 Description

💱 This is a Currency Converter Web Application built using HTML, CSS, and JavaScript.

🌍 The application uses a real-time Exchange Rate API to convert currencies instantly and accurately.

⚡ It provides a clean, responsive, and user-friendly interface for quick currency conversion between multiple currencies.

🛠️ Technologies Used

🌐 HTML5 – Structure and Content
🎨 CSS3 – Styling and Flexbox Layout
⚡ JavaScript (ES6+) – Dynamic Functionality
🔗 Fetch API – HTTP Requests and API Integration
💱 Exchange Rate API – Real-Time Currency Data

🎨 UI Design

🎯 Clean and Minimal Layout
📦 Centered Card-Style Container
📱 Responsive Flexbox Design
✨ Smooth User Experience
📊 Highlighted Result Display Section

✨ Features

💰 Enter Amount to Convert
🌍 Select Currencies Dynamically
🔄 Convert Between Multiple Currencies
⚡ Real-Time Exchange Rates
📊 Instant Conversion Results
🎯 Default Conversion: USD → INR
✅ User-Friendly Interface
📱 Responsive Design for All Devices
